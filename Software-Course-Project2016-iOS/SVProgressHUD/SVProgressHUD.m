...
+ (void)setTitleFont:(UIFont*)font {
    [self sharedView].titleFont = font;
}
...
+ (void)showTitle:(NSString*)title status:(NSString*)status {
    NSTimeInterval displayInterval = [self displayDurationForString:status];
    [[self sharedView] showTitle:title status:status duration:displayInterval];
}
...
- (instancetype)initWithFrame:(CGRect)frame{
    if((self = [super initWithFrame:frame])){
        ...
        if ([UIFont respondsToSelector:@selector(preferredFontForTextStyle:)]) {
            ...
            _titleFont = [UIFont preferredFontForTextStyle:UIFontTextStyleCaption1];
        } else {
            _font = [UIFont systemFontOfSize:14.0f];
            _titleFont = [UIFont boldSystemFontOfSize:24];
        }
...
- (void)updateHUDFrame{
    ...
    BOOL titleUsed = ![self.titleLabel isHidden];
...
#pragma clang diagnostic pop
        ...
        if(imageUsed || progressUsed || titleUsed){
    ...
    if (titleUsed && title) {
        ...
        if([title respondsToSelector:@selector(boundingRectWithSize:options:attributes:context:)]){
            titleRect = [title boundingRectWithSize:constraintSize
                                            options:(NSStringDrawingOptions)(NSStringDrawingUsesFontLeading|NSStringDrawingTruncatesLastVisibleLine|NSStringDrawingUsesLineFragmentOrigin)
            ...
            if([title respondsToSelector:@selector(sizeWithAttributes:)]){
                titleSize = [title sizeWithAttributes:@{NSFontAttributeName:[UIFont fontWithName:self.titleLabel.font.fontName size:self.titleLabel.font.pointSize]}];
            } else{
#if TARGET_OS_IOS
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated"
                titleSize = [title sizeWithFont:self.titleLabel.font constrainedToSize:CGSizeMake(200.0f, 300.0f)];
#pragma clang diagnostic pop
#endif
            }
            titleRect = CGRectMake(0.0f, 0.0f, titleSize.width, titleSize.height);
        }
    ...
    if(string){
        ...
        self.titleLabel.center = CGPointMake(CGRectGetWidth(self.hudView.bounds)/2, 36.0f);
    } else{
       	self.imageView.center = CGPointMake(CGRectGetWidth(self.hudView.bounds)/2, CGRectGetHeight(self.hudView.bounds)/2);
        self.titleLabel.center = CGPointMake(CGRectGetWidth(self.hudView.bounds)/2, CGRectGetHeight(self.hudView.bounds)/2);
    }
...
- (void)showTitle:(NSString*)title status:(NSString*)string duration:(NSTimeInterval)duration {
...
- (UILabel*)titleLabel{
    ...
    _titleLabel.textColor = self.foregroundColorForStyle;
...
- (void)setTitleFont:(UIFont *)titleFont{
    if (!_isInitializing) _titleFont = titleFont;
}
